...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    room.articles.forEach(
      (article: {
        title: string;
        author?: string | null;
        source_url: string;
        status: string;
        highlights: { content: string; note?: string | null }[];
      }) => {
    ...
    room.vaultTrails.forEach(
      (trail: {
        vault_entry: { term: string; definition?: string | null; user_note?: string | null };
        passage?: string | null;
      }) => {
