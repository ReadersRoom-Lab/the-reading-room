...
function generateContributions() {
  try {
    // Construct a safe, unwriteable system PATH to satisfy security constraints
    ...
    const logOutput = execSync('git log --format="%ad|%an|%s" --date=short --no-merges', { 
      encoding: 'utf-8',
      env: { PATH: systemPath }
    });
    
    const lines = logOutput.trim().split('\n').filter(Boolean);
    const grouped = parseGitLog(lines);
