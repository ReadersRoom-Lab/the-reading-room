...
function HighlightAd({ isActive }: Readonly<{ isActive: boolean }>) {
  ...
  let highlightClass =
    "bg-transparent text-gray-700 border-b-2 border-transparent px-1 rounded transition-colors duration-300";
  if (isSelected) {
    highlightClass =
  ...
  return (
    <div className="p-6 relative h-full flex flex-col justify-center">
      ...
      <p className="font-serif text-xs leading-relaxed text-gray-700 mb-2 transition-colors">
        When we interact with complex systems, our minds naturally seek to build{" "}
        <mark className={highlightClass}>structural scaffolding</mark>. This allows us to map new
        concepts onto existing frameworks.
      </p>
      <p className="font-serif text-xs leading-relaxed text-gray-700">
        The most effective learning environments are those that{" "}
        <mark className="bg-[#D1D9D3]/40 text-black px-1 rounded border-b-2 border-[#D1D9D3]">
          encourage active synthesis
        </mark>{" "}
        rather than passive consumption. By highlighting and annotating, we transform the text into
        our own context.
      </p>
      ...
      >
        <div
          className={cn(
            "w-5 h-5 rounded-full bg-[#E4D7C5] transition-transform duration-300",
            step === 3 && "scale-125 shadow-md"
          )}
        />
  ...
  return (
    <div className="p-6 h-full flex flex-col justify-center relative">
      <div className="flex items-center gap-2 mb-4">
        <Sparkles
          className={cn(
            "w-5 h-5 text-indigo-500 transition-all duration-1000",
            step > 0 ? "animate-pulse shadow-[0_0_20px_rgba(99,102,241,0.5)]" : ""
          )}
        />
      ...
      <div className="space-y-4">
        <div className="h-2 bg-gray-100 rounded w-full overflow-hidden relative">
          <div
            className="absolute top-0 left-0 h-full bg-indigo-200 transition-all duration-1000 ease-out"
            style={{ width: step > 0 ? "100%" : "0%" }}
          />
        </div>
        <div className="h-2 bg-gray-100 rounded w-5/6 overflow-hidden relative">
          <div
            className="absolute top-0 left-0 h-full bg-indigo-200 transition-all duration-1000 ease-out delay-150"
            style={{ width: step > 1 ? "100%" : "0%" }}
          />
        </div>
        <div className="h-2 bg-gray-100 rounded w-4/6 overflow-hidden relative">
          <div
            className="absolute top-0 left-0 h-full bg-indigo-200 transition-all duration-1000 ease-out delay-300"
      ...
      <div
        className={cn(
          "absolute bottom-6 left-6 right-6 p-3 bg-indigo-50/80 backdrop-blur-sm border border-indigo-100 rounded-lg text-xs font-serif italic text-indigo-900 transition-all duration-700",
          step === 3 ? "opacity-100 translate-y-0 shadow-lg" : "opacity-0 translate-y-4"
        )}
      >
        &quot;A profound conceptual link was found between your highlights on <b>Philosophy</b> and{" "}
  ...
  return (
    <div className="flex h-full w-full">
      <div className="w-1/3 border-r border-gray-100 p-4 bg-gray-50/50 flex flex-col gap-2 relative z-10">
        ...
        <div
          className={cn(
            ...
            step === 1 ? "bg-white shadow-md ring-1 ring-blue-200 scale-105" : ""
          )}
        >
      ...
      <div className="w-2/3 p-6 relative bg-white overflow-hidden">
        ...
        <div
          className={cn(
            "p-4 border border-gray-200 shadow-sm rounded-lg bg-white flex items-start gap-3 absolute top-6 left-6 right-6 transition-all duration-700 ease-in-out z-20",
            dragFileClass
          )}
        >
        ...
        <div
          className={cn(
            "p-4 border border-gray-100 border-dashed rounded-lg bg-gray-50 flex items-start gap-3 absolute top-28 left-6 right-6 transition-all duration-700 ease-in-out",
...
export function LandingMockup() {
  ...
  const titles = ["Read & Highlight", "Synthesize with AI", "Organize in Rooms"];

  return (
    <div
      className="bg-gray-50/50 p-6 lg:p-8 relative overflow-hidden flex items-center justify-center border-t border-[#E5E5E5] w-full min-h-[360px]"
      aria-hidden="true"
    >
      {/* Glow effect behind the mockup */}
      <div
        className={cn(
          "absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[500px] h-[300px] blur-[80px] rounded-full pointer-events-none transition-colors duration-1000",
          ...
          <div className="relative w-full h-[220px] bg-white">
            <div
              className={cn(
                ...
                activeAd === 0 ? "opacity-100 z-10" : "opacity-0 z-0 pointer-events-none"
              )}
            >
            ...
            <div
              className={cn(
                "absolute inset-0 transition-opacity duration-1000",
                activeAd === 1 ? "opacity-100 z-10" : "opacity-0 z-0 pointer-events-none"
              )}
            >
            ...
            <div
              className={cn(
                "absolute inset-0 transition-opacity duration-1000",
                activeAd === 2 ? "opacity-100 z-10" : "opacity-0 z-0 pointer-events-none"
              )}
            >
