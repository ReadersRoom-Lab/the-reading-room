...
export function LibraryImportDialog({
  ...
  roomId: string;
  libraryArticles: { id: string; title: string; source_url: string }[];
}>) {
            ...
            <div className="h-[300px] overflow-y-auto overflow-x-hidden">
              <div className="flex flex-col">
                {articles.map((article) => (
                  <div
                    key={article.id}
                    className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 py-4 pl-4 pr-6 border-b border-border/50 last:border-0 hover:bg-muted/30 transition-colors"
                  >
